function t = testCPDComentadoPair(currPath, filepath, ext, param, paramCorner, show, method, nDim, readData, saveData, opt, stat, statPath, errPath)
...
[imgName1, imgName2] = textread(filepath, '%s %s');
    
imgName1(:)=strcat(currPath,imgName1(:),ext);
imgName2(:)=strcat(currPath,imgName2(:),ext);

% Recorremos todas las imágenes del fichero
for i=1:length(imgName1)        
    try
        imName = char(imgName1(i))
        img1=imread(imName);
        img1=rgb2gray(img1);
        ...
        matName=strcat(imgName1(i), '.', imgName2(i), '.', method, num2str(nDim), 'D', 'param', num2str(param), '.mat');
