function t = testCPDComentado(currPath, filepath, ext, param, paramCorner, show, method, nDim, readData, saveData, opt, stat, statPath, errPath)
...
if (strmatch('fast', method))
    ...
    corners1 = fast_nonmax(double(img1), param, corners1);
...
%         end
        ...
        if (strmatch('fast', method))
            ...
            corners2 = fast_nonmax(double(img2), param, corners2);
...
%             corners2=[cout(:,2), cout(:,1)];
        ...
        if (readData == 0)
            try
                ...
                if (stat)
                    ...
                    fprintf(fid, '%s\t%f\t%f\t%f\t%s\t%d\t%s\t%d\t%d\t%d\t%f\n', char(currPath), dist(i), ang(i), param, method, nDim, opt.method, 0, 0, 0, 0);
                    fclose(fid);
                end
            end
        end
        ...
        matName=strcat(imgName(i), '.', method, num2str(nDim), 'D', 'param', num2str(param), '.mat');
        ...
        if (stat)
            fid=fopen(statPath, 'a');
            fprintf(fid, '%s\t%f\t%f\t%f\t%s\t%d\t%s\t%d\t%d\t%d\t%f\n', char(currPath), dist(i), ang(i), param, method, nDim, opt.method, length(C), diffs, nMask, telapsed);
            fclose(fid);
        end
        ...
        fid=fopen(errPath, 'a');                
        fprintf(fid, '%s\t%f\t%s\t%d\t%s\t%s\t%f\t%f\n', char(currPath), param, char(method), nDim, char(opt.method), em.identifier, dist(i), ang(i));
        fclose(fid);
    end    
end
