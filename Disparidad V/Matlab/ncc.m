...
maxD = floor ((frameSize(2) / 3) / 2);   % Valor máximo para la disparidad
%maxD = 500;

mt = mean(mean(template));      % Media del patrón
vart = sum(sum(template .* template)) / N - (mt * mt);  % Varianza del patrón
...
[dummy, disparity] = max(correlation(first:last));      % Disparidad = Posición del máximo valor de correlación
