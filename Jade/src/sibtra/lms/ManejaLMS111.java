...
import sibtra.lidar.PanelMuestraBarridoAngular;
 ...
 *
 */
public class ManejaLMS111 implements Lidar {
...
//			ne.printStackTrace();
	...
	double str2real(String st) {
		st.trim();
		try{
			Long i = Long.parseLong(st, 16);
			if(st.length()==8)
				return (double)Float.intBitsToFloat(i.intValue());
			else
				return Double.longBitsToDouble(i);			
		} catch (NumberFormatException ne) {
			System.err.println("Error al convertir a real "+st+"\n"+ne.getLocalizedMessage());
			ne.printStackTrace();
		}
		return Double.NaN;
	}
	...
	public BarridoAngular111 parseaBarrido(String resp) {
		BarridoAngular111 bAngDevol=null;
		if(!resp.startsWith("sRA LMDscandata")
				&& !resp.startsWith("sSN LMDscandata")) return bAngDevol; //no es barrido
		...
		for(int i=0; i<numChanels16; i++) {
			String measureDataContent=campos[ca++];
			double scalingFactor=str2real(campos[ca++]);
			double scalingOffset=str2real(campos[ca++]);
			int startingAngle=str2int(campos[ca++]);
			int angularStep=str2int(campos[ca++]);
			
			log+="\n"+ca+": Chan"+i+" DataCont="+measureDataContent+" ScanFact="+scalingFactor
			+" ScanOffset="+scalingFactor+" StarAng="+startingAngle+" AngStep="+angularStep;
			int numData=str2int(campos[ca++]);
			log+="\n\t"+(ca-1)+": "+"NumData="+numData;
			if(measureDataContent.equals("DIST1")) {
				//obtenemos el barrido
				int[] datos=new int[numData];
				for(int j=0; j<numData; j++)
					datos[j]=str2int(campos[ca++]);
				bAngDevol=new BarridoAngular111(datos,startingAngle,angularStep,scalingFactor);
			} else
				ca+=numData;
		}
	...
	public static void main(String[] args) {
		...
		try {
			SalvaMATv4 sam4=new SalvaMATv4("/tmp/Barrido.mat");
			ba.savalAMatv4(sam4,"barrido1");
			sam4.close();
		} catch (IOException e) {
			System.err.println("Problama al salvar barrido en fichero MAT:"+e.getLocalizedMessage());
		}
		System.exit(0);
