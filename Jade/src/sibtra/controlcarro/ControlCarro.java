...
CommPortOwnershipListener {
	 ...
	 * Valor_Freno = indica la fuerza, la amplitud con la que se abre la valvula
	 *               que abre el circuito de frenado.
	 * Tiempo_freno = Tiempo en el que esta actuando el freno, funciona a la inversa
	 *                donde el tiempo que se pone tiene que saturar a MaxpasosFreno,FFh 
	 *                para parar el frenado. MaxpasosFreno = 10 decimal, EN REALIDAD 
	 *                SE FIJA EN EL MICRO LA PARTE ALTA DE LA PALABRA A MAXPASOSFRENO -1
	 *                POR LO QUE SE HACE SOLO LA CUENTA BAJA. Hay que ajustar los parametros
	 *                de tiempo para conseguir que funcione correctamente
	 * / */
...
//	public static final double RADIANES_POR_CUENTA = 2*Math.PI/MAX_CUENTA_VOLANTE;
	...
	public ControlCarro(String portName) {
		...
		parameters = new ControlCarroSerialParameters(portName, 9600, 0, 0, 8, 1, 0);
		try {
			openConnection();
   ...
   to reliquish the port if have it open and no longer need it.
	...
	private void openConnection() throws ControlCarroConnectionException {
		...
		try {
			...
			sPort.close();
			throw new ControlCarroConnectionException("Error opening i/o streams");
		}
...
//		messageAreaOut.addKeyListener(keyHandler);
		...
		try {
			...
			sPort.close();
			throw new ControlCarroConnectionException("too many listeners added");
		}
...
//		}
    ...
    throw exception.
	 */
	private void setConnectionParameters() throws ControlCarroConnectionException {
		...
		try {
			...
			throw new ControlCarroConnectionException("Unsupported flow control");
		}
	}
...
//		messageAreaOut.removeKeyListener(keyHandler);
	...
	/**
    Devuelve si esta pulsada la alarma global del coche, en este caso 
	 * el sistema bloquea todas las posibles opciones
    ...
    Da N pasos de desfrenado para ir aguantando el coche poco a poco.
	 ...
	 /**
	  * Funci�n para el calculo de la velocidad a partir del encoder incluido en el vehiculo.
	  */
...
public void setConsigna(double valor) {
    ...
    if (error >= 0) {
	 ...
	 /**
	  * Fija el valor de la velocidad de avance en Metros Segundo y activa el control
	  * @param valor consigna en metros/seg
	  */
	 ...
	 /**
	  * Fija el valor de la velocidad de avance en Kilometros hora y activa el control
	  * @param valor consigna en Kilometros hora
	  */
	 ...
	 /**
	  * Fija el incremento m�ximo entre dos comandos de tracci�n consecutivos para
	  * evitar aceleraciones muy bruscas
	  * @param incremento diferencia m�xima entre dos comandos
	  */
