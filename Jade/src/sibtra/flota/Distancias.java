...
static String assertLimpiezaDistanciasConflictos = "(assert (triple (predicate http://www.isaatc.ull.es/Verdino.owl#Accion) (subject   http://www.isaatc.ull.es/Verdino.owl#AccionActual) (object   http://www.isaatc.ull.es/Verdino.owl#LimpiarDistanciasConflictos) ))";
	...
	{   // hacemos copias locales
		...
		cargarHechosReglas();
	// assert tramos (leerlos de ontologias) y preparar la ontolog�a del c�lculo de rutas
   ...
   { System.out.println("calculaVectorTramosDeOntologia");
    ...
    Iterator result2 = engine.runQuery("buscaSujetosConObjeto", vector);
	  ...
	  vehiculo.fijaPosicionVehiculo(posicion);
		...
		inicializacionFicticia();  //m�todo a quitar en situaciones reales
	}
	...
	//m�todo a cambiar cuando funcione el GPS... en realidad, la clase vehiculo
	...
	Vehiculo actual = (Vehiculo) hashVehiculos.get(id);
	 return actual.dimeVelocidad();
		...
		//m�todo a cambiar cuando funcione el GPS
 ...
 vector.add(new Value("http://www.isaatc.ull.es/Verdino.owl#tieneLongitud", RU.ATOM));
  ...
  if((openFile)!= null) 
   {try
	...
	// se supone que este m�todo ser� llamado al principio y acorde con la situaci�n real
...
// escribir las prioridades en la ontolog�a	   
...
{engine.run();
  ...
  {waiting(1000);
    ...
    }  // fin del for de c�lculo de nuevas posiciones
	for (int i=0; i<sizeVector ; i++)
	{Vehiculo actual = (Vehiculo) (vectorVehiculos.elementAt(i));
	 // assert posicion
