 ...
 * @author alberto
 */
public class LoggerArrayDoubles extends Logger {
	
	private Vector<double[]> datos=null;
	
	/** Nos vale el constructor de la superclase */
	LoggerArrayDoubles(Object objeto, String nombre, long t0, int numMuestras) {
		super(objeto, nombre, t0, numMuestras);
	}

	/** Nos vale el constructor de la superclase */
	LoggerArrayDoubles(Object objeto, String nombre, long t0) {
	...
	void activa(int duracionSg) {
		...
		if(datos==null)
			datos=new Vector<double[]>(minCapacity);
		else
	...
	public void add(double[] dato) {
		...
		if(!activado)
			return;
		//copiamos Datos
		double[] nD=new double[dato.length];
	...
	/** Crea nuevo array con los datos pasados, lo añade y apunta instante de tiempo */
	public void add(double d1, double d2, double d3) {
		super.add();
		if(!activado)
			return;
		//copiamos Datos
	...
	public void add(double d1, double d2, double d3, double d4) {
		...
		if(!activado)
			return;
		//copiamos Datos
		double[] nD={d1,d2,d3,d4};
		datos.add(nD);
	}

	/** Crea nuevo array con los datos pasados, lo añade y apunta instante de tiempo */
	public void add(double d1, double d2, double d3,double d4, double d5) {
		super.add();
		if(!activado)
			return;
		//copiamos Datos
		double[] nD={d1,d2,d3,d4,d5};
		datos.add(nD);
	}

	/** Crea nuevo array con los datos pasados, lo añade y apunta instante de tiempo */
	public void add(double d1, double d2, double d3,double d4, double d5,double d6) {
		super.add();
		if(!activado)
			return;
		//copiamos Datos
		double[] nD={d1,d2,d3,d4,d5,d6};
		datos.add(nD);
	}

	/** Crea nuevo array con los datos pasados, lo añade y apunta instante de tiempo */
	public void add(double d1, double d2, double d3,double d4, double d5,double d6,double d7) {
		super.add();
		if(!activado)
			return;
		//copiamos Datos
		double[] nD={d1,d2,d3,d4,d5,d6,d7};
		datos.add(nD);
	}

	/** Crea nuevo array con los datos pasados, lo añade y apunta instante de tiempo */
	public void add(double d1, double d2, double d3,double d4, double d5,double d6,double d7,double d8) {
		super.add();
		if(!activado)
			return;
		//copiamos Datos
		double[] nD={d1,d2,d3,d4,d5,d6,d7,d8};
		datos.add(nD);
	}
			...
			for(int i=0; i<nFil; i++) {
				...
				for(int j=0; j<nCol; j++)
					st+=(j<datos.get(i).length
							?String.format((Locale)null,"\t%f", datos.get(i)[j])
							:"\tNaN");
				st+="\n";
			}
		return st;
	...
	void vuelcaMATv4(SalvaMATv4 smv4) throws IOException {
		...
		smv4.matrizDoubles(datos, nombre);
	}
