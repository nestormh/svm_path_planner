...
//			tendenciaRepulsion = false;
	...
	/**MÃ©todo optimizado para calcular la cohesion, alineacion y separacion para un boid. 
...
//		velResultante = velResultante.times(1/velResultante.norm2());
	 ...
	 *  con sus compaÃ±eros de bandada*/
...
//			System.out.println("DespuÃ©s de dividir y cont vale " +cont);
//			centroMasa.print(10,4);
		}
		else{ // si no hay vecinos nos quedamos con la posicion del boid
			centroMasa = this.getPosicion();
//			System.out.println("No habÃ­a vecinos");
//			centroMasa.print(10,4);
		}
...
//						,Boid.getObjetivo().get(0,0),Boid.getObjetivo().get(1,0));
		for (int i=0;i < obstaculos.size();i++){
			...
			if (dist < radioObstaculo){
				...
				//--Si el obstáculo tiene velocidad lo esquivamos dinamicamente, si está parado usamos la tendencia
...
//				if (UtilCalculos.diferenciaAngulos(angDirecObjetivo, angDirecBoidObstaculo)< Math.PI/2){
					...
					if(obstaculos.elementAt(i).velocidad.norm2() != 0){// Tiene velocidad						
						...
						}else{//Si no va a cruzarse con el obstaculo no se le aÃ±ade compensacion lateral
							//ni repulsion
							c = c.plus(repulsion);
							//							c = c.plus(cero);
						}									
...
//						repulsion = repulsion.plus(compensacion);
	...
	/** MÃ©todo que calcula todas las reglas para cada Boid, las suma vectorialmente
