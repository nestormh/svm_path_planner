...
public class Simulador{
	...
	/** Vector donde se almacenan los boids vecinos a una determianda part�cula*/
	Vector<Boid> particulasVecinas = new Vector<Boid>();
	/** Vector donde se almacenan los obst�culos vecinos para una determinada part�cula*/
...
//			System.out.println("TamaÃ±o actual de la bandada " + getBandada().size());			
			...
			for (int j = 0;j<getBandada().size();j++){
				...
				//-----------A�ADO CODIGO PARA LA OPTIMIZACION DE LA BUSQUEDA DE VECINOS------------------------
				
				
				//La part�cula se encuenta dentro del escenario, por lo que buscamos el gridSearchPoint de la rejilla uniforme que le corresponde y la apuntamos
				if(getBandada().elementAt(j).getPosicion().get(0,0) < getLargoEscenario() && getBandada().elementAt(j).getPosicion().get(0,0) > 0 &&
					...
					int indiceY = (int)Math.floor(getBandada().elementAt(j).getPosicion().get(1,0)/getRejilla().getResolution());
...
//					System.out.println("tama�o bandada "+ this.getBandada().size()+"boid "+j);
...
//					System.out.println("posici�n del boid, x= "+ getBandada().elementAt(j).getPosicion().get(0,0)+" y= "+getBandada().elementAt(j).getPosicion().get(1, 0));
					

				}
//				else{ // si est� fuera del escenario eliminamos la part�cula
//					getBandada().remove(j);
//				}
...
//						,getObstaculos(),j,Boid.getObjetivo());
					...
					//-----------------------------------------------------------------------------------
					//------------A�ADO M�TODOS PARA MEJORAR EL RENDIMIENTO DEL ALGORITMO-----------------
					//-----------------------------------------------------------------------------------
					
					// Busco las part�culas vecinas, para ello se calcula la celda en la que se encuentra la part�cula en estudio y el offset de celdas en el que se
					// quiere comprobar si hay vecinas
					
					this.particulasVecinas.clear();
					this.obstaculosVecinos.clear();
					
					this.particulasVecinas = getRejilla().buscarParticulasVecinas((int)Math.floor(getBandada().elementAt(j).getPosicion().get(0,0)/getRejilla().getResolution()),
					...
					this.obstaculosVecinos = getRejilla().buscarObstaculosVecinos((int)Math.floor(getBandada().elementAt(j).getPosicion().get(0,0)/getRejilla().getResolution()),
							(int)Math.floor(getBandada().elementAt(j).getPosicion().get(1,0)/getRejilla().getResolution()), 
							(int)Math.floor(Boid.radioObstaculo/getRejilla().getResolution()));
//					System.out.println("tama�o del vector de particulas vecinas " + particulasVecinas.size());
...
//				}
			...
			//----------BORRO LAS PART�CULAS Y OBST�CULOS APUNTADAS EN LOS GRIDsEARHpOINT DE LA REJILLA UNIFORME----
...
//		}
	...
	public void moverObstaculos(){
		if(getObstaculos().size() != 0){
			...
			//Borro los obst�culos apuntados en los gridSearhPoint de la rejilla uniforme
...
//				System.out.println("n�mero de obst�culos "+this.getObstaculos().size()+"obstaculo "+i);
				if(indiceX >= 0 && indiceX <= getRejilla().getNumPtosX()-1 && indiceY >= 0 && indiceY <= getRejilla().getNumPtosY()-1 ){
...
//				
//				
//				//El obst�culo se encuenta dentro del escenario, por lo que buscamos el gridSearchPoint de la rejilla uniforme que le corresponde y la apuntamos
