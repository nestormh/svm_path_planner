...
map<const char *, manejador_t> listaDispositivos;
...
vlc_module_begin();
	...
	add_integer( "dshow-alto",  -1, NULL, VIDEO_IN_TEXT,
                 VIDEO_IN_LONGTEXT, VLC_TRUE );
                       ...
                       vlc_bool_t b_access_demux )
{
    ...
    {
		if (strcmp(vdevname.c_str(), "none") != 0) 
    ...
    {
		if (strcmp(adevname.c_str(), "none") != 0) 
...
static void CommonClose( vlc_object_t *p_this, access_sys_t *p_sys )
{
	...
	try {
		HANDLE hMapFile;
		LPCTSTR pBuf;
		for( map<const char*, manejador_t>::iterator iter = listaDispositivos.begin(); 
			iter != listaDispositivos.end(); iter++ ) {

				manejador_t manejador = iter->second;
				hMapFile = manejador.hMapFile;
				pBuf = manejador.pBuf;

				msg_Err(p_this, "Liberando %s", manejador.devicename.c_str());
				msg_Err(p_this, "Libera el mapeo del fichero");
				if (pBuf != NULL)
					UnmapViewOfFile((void *)pBuf);
				msg_Err(p_this, "Libera el manejador");
				if (hMapFile != NULL)
					CloseHandle(hMapFile);
		}
	} catch(char * str) {
		msg_Err(p_this, "Error al liberar la memoria, %s", str);
	}
	// Fin de miCodigo
                       ...
                       string devicename, vlc_bool_t b_audio )
{
    ...
    {
		if (strcmp(devicename.c_str(), "none") != 0) {		
	...
	// miCodigo
	// Lo ponemos justo antes de EnumDeviceCaps, que es donde estoy obteniendo
	// el ancho, alto, etc, del dispositivo. Si lo pusiera antes o después, no
	// puedo asegurar que la variable dispositivo tenga el valor adecuado
	try {
		if ((strcmp(devicename.c_str(), "default") != 0)
			&& (strcmp(devicename.c_str(), "none") != 0)) {

			dispositivo = devicename;
		}
	} catch (char * str) {
		msg_Err(p_this, "Excepción al obtener el nombre del dispositivo, %s", str);
	}
	// Fin de mi Codigo
    ...
    if( ConnectFilters( p_this, p_sys, p_device_filter, p_capture_filter ) )
    {
        ...
        if( dshow_stream.i_fourcc )
        {
			...
			try {
				...
				msg_Err(p_this, "Error al asignar nombre del dispositivo, %s", str);
			}
			// Fin de miCodigo
    ...
    while( p_class_enum->Next( 1, &p_moniker, &i_fetched ) == S_OK )
    {
        ...
        if( SUCCEEDED(hr) )
        {
            ...
            if( SUCCEEDED(hr) )
            {
                ...
                char *p_buf = (char *)alloca( i_convert+1+2 ); p_buf[0] = 0;
                WideCharToMultiByte( CP_ACP, 0, var.bstrVal,
                SysStringLen(var.bstrVal), p_buf, i_convert, NULL, NULL );
                SysFreeString(var.bstrVal);
                p_buf[i_convert] = ':';
                char * mi_index = new char;
                itoa(indice, mi_index, 10);
                p_buf[i_convert+1] = *mi_index;                
                p_buf[i_convert+2] = '\0';

                if( p_listdevices ) p_listdevices->push_back( p_buf );

                /*msg_Err(p_this, p_buf, 1);
                if (p_devicename)
                	msg_Err(p_this, p_devicename->c_str(), 1);*/
        ...
        */
        ...
        while( p_enummt->Next( 1, &p_mt, NULL ) == S_OK )
        {
                ...
                && p_mt->formattype == FORMAT_VideoInfo )
            {
                ...
                msg_Err( p_this, "EnumDeviceCaps: input pin "
				...
				// miCodigo
				msg_Err(p_this, "Ancho: %i, Alto: %i, Croma: %4.4s, fps: %f", i_current_width, 
					i_current_height, (char *)&i_current_fourcc, (10000000.0f/((float)i_current_atpf)));				
				
				msg_Err( p_this, "Inicio de memoria compartida");				
				
				// Recorremos el map a ver si ya tenemos ese dispositivo en memoria
				try {
					map<const char *, manejador_t>::iterator iter = listaDispositivos.find(dispositivo.c_str());	
					if (iter != listaDispositivos.end()) {
						 msg_Err( p_this, "Error, el dispositivo %s ya ha creado memoria compartida", dispositivo.c_str());
					} else {
						int imagesize = i_current_width * i_current_height * 3;
						msg_Err( p_this, "Memoria reservada: %i", imagesize);
						int tamano = sizeof(int) + sizeof(int) + sizeof(char) * 4 +
							imagesize * sizeof(unsigned char);
			
						msg_Err( p_this, "Creando fichero de memoria con nombre %s", dispositivo.c_str());
					
					
						// Crea el fichero de memoria con nombre igual al dispositivo a capturar
						HANDLE hMapFile = CreateFileMapping(
							INVALID_HANDLE_VALUE,    // Usa memoria, no disco
							NULL,                    // default security 
							PAGE_READWRITE,          // read/write access
							0,                       // max. object size 
							tamano,         // buffer size  
							dispositivo.c_str());    // name of mapping object
     
						if (NULL == hMapFile || 
							INVALID_HANDLE_VALUE == hMapFile) 
						{ 
							msg_Err( p_this, "Error al crear el fichero de memoria, %l", GetLastError());
							return 0;
						}

						msg_Dbg( p_this, "Abriendo fichero de memoria");
						LPCTSTR pBuf = (LPTSTR)
						...
						if (NULL == pBuf) 
						{ 
							msg_Err( p_this, "Error al mapear el fichero de memoria, %l", GetLastError());
							return 0;
						}
						int posicion = 0;
						memcpy((LPVOID)&(pBuf[posicion]), &i_current_width, sizeof(i_current_height));
						posicion += sizeof(i_current_width);
						memcpy((LPVOID)&(pBuf[posicion]), &i_current_height, sizeof(i_current_height));
						posicion += sizeof(i_current_height);		
						memcpy((LPVOID)&(pBuf[posicion]), (char *)&i_current_fourcc, sizeof(char) * 4);
						posicion += sizeof(char) * 4;
	
						unsigned char * miImagen = new unsigned char[imagesize];

						memcpy((LPVOID)&(pBuf[posicion]), miImagen, imagesize * sizeof(unsigned char));
				
						msg_Err( p_this, "Fin de memoria compartida");
						...
						listaDispositivos[dispositivo.c_str()] = manejador;
						msg_Err( p_this, "Añadido manejador a la tabla hash");
					}
												
				} catch (char * str) {
					msg_Err( p_this, "Error al crear la memoria compartida. Esto puede derivar en sucesivos errores no controlados, %s", str);
				}
				// Fin de miCodigo				
            }
...
static int Demux( demux_t *p_demux )
{
	...
	if (strcmp(p_stream->devicename.c_str(), "Error") != 0) {
		try {
			map<const char *, manejador_t>::iterator iter = listaDispositivos.find((p_stream->devicename).c_str());	
			if (iter == listaDispositivos.end()) {
				msg_Err( p_demux, "Error, el dispositivo %s no ha creado memoria compartida", (p_stream->devicename).c_str());
				p_stream->devicename="Error";			
			} else {		
				manejador_t manejador = iter->second;
				if (manejador.pBuf == NULL) {
					msg_Err( p_demux, "Error, el dispositivo %s aún tiene el buffer vacío", (p_stream->devicename).c_str());
				} else {
					LPCTSTR pBuf;
					int posicion;
			
					pBuf = manejador.pBuf;
					posicion = manejador.posicion;

					CopyMemory((LPVOID)&(pBuf[posicion]), p_data, i_data_size * sizeof(unsigned char));
				}
			}
		} catch (char * str) {
			msg_Err(p_demux, "Excepción al obtener el frame: %s", str);
		}
	}
	// Fin de miCodigo
