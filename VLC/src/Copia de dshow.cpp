...
{	
    ...
    {
		if( !b_audio )
			msg_Err( p_this, "No se encontraron dispositivos de vídeo" );
		/*else
    ...
    while( p_class_enum->Next( 1, &p_moniker, &i_fetched ) == S_OK )
    {
        ...
        if( SUCCEEDED(hr) )
        {
            ...
            if( SUCCEEDED(hr) )
            {
				...
				char *p_buf2 = (char *)alloca( i_convert+1 ); p_buf2[0] = 0;
                WideCharToMultiByte( CP_ACP, 0, var.bstrVal,
                SysStringLen(var.bstrVal), p_buf2, i_convert, NULL, NULL );
                SysFreeString(var.bstrVal);                
				char *p_buf;
				if (p_devicename != NULL) {
					if ((strcmp(p_devicename->c_str(), "none") == 0) || (strcmp(p_devicename->c_str(), "None") == 0) ||
						(strcmp(p_devicename->c_str(), "default") == 0) || (strcmp(p_devicename->c_str(), "Default") == 0) ||
						(strcmp(p_devicename->c_str(), "") == 0)) {
						msg_Err(p_this, "Entrando en none", 1);	
						p_buf = p_buf2;
						p_buf[i_convert] = '\0';
					}
				} else {
					msg_Err(p_this, "Entrando en else", 1);	
					p_buf = new char[i_convert+3];
					for (int i = 0; i < i_convert; i++) {
						p_buf[i] = p_buf2[i];
					}
					p_buf[i_convert] = ':';
