...
void IncrementKernelCache(vector<int>& KernelCacheItersSinceUsed,const int &RowsInKernelCache);
...
extern "C"
void SVRTrain(float * elapsed, float *alpha,float* beta,float*y,float *x, float C, float kernelwidth, float eps, int m, int n, float StoppingCrit)
{
    ...
    kernelwidth*=-1;
    CUDA_SAFE_CALL(cudaMemcpyToSymbol("kernelwidth", &kernelwidth, sizeof(float)));

    CUDA_SAFE_CALL(cudaMemcpyToSymbol("C", &C, sizeof(float)));
    ...
    for(int k=0;k<m;k++)
    {
        alpha[k]=(alphasvr[k]-alphasvr[k+m])*ybinary[k];
    }
...
extern "C"
void SVMTrain(float * elapsed, float *alpha,float *beta, float *y,float *x, 
              float C, float kernelwidth, int m, int n, float StoppingCrit) {

    std::cout << "SVMTrain " << __LINE__ << std::endl;
...
//     cublasGetVector(m,sizeof(float),d_alpha,1,alpha,1);
    CUDA_SAFE_CALL(cudaMemcpy(h_alpha, d_alpha, m * sizeof(float), cudaMemcpyDeviceToHost));
