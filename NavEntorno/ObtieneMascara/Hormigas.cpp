...
CvPoint * Hormigas::obtieneMascara(IplImage * img, secuenceStruct * left, secuenceStruct * right, int * total) {
	
	if (! hormigas) {
		initHormigas(cvGetSize(img));
	}		
	...
	setPointofAttraction(mask,shadowMask, &shortestLeftPath, &shortestLeftPath,&attractionX,&aRef,&bRef,&cRef,&dRef,&corte,consigna);		
	...
	*total = shortestLeftPath.top + shortestRightPath.top;	

	//cout << *total << endl;

	if (*total <= 0) return NULL;
	
	CvPoint * retorno = new CvPoint[(*total)];
	for (int i = 0; i < shortestLeftPath.top; i++) {
		//cout << "i= " << i << endl;
		retorno[i] = cvPoint(shortestLeftPath.sec[i].i, 240 - shortestLeftPath.sec[i].j);
	}

	//cout << "***" << endl;

	int j = 0;
	for (int i = shortestLeftPath.top + shortestRightPath.top - 1; i >= shortestLeftPath.top; i--) {
		//cout << "i= " << i << endl;
	//for (int i = 0; i < shortestRightPath.top; i++) {
		retorno[i] = cvPoint(shortestRightPath.sec[j].i, 240 - shortestRightPath.sec[j].j);
		j++;
	}
...
/*	
tMascara Hormigas::obtieneMascara(IplImage * img, secuenceStruct * left, secuenceStruct * right) {
	...
	setPointofAttraction(mask,shadowMask, &shortestLeftPath, &shortestLeftPath,&attractionX,&aRef,&bRef,&cRef,&dRef,&corte,consigna, &puntos);		

	cvAdd(img, mask, mask);	

	for (int i = 0; i < shortestLeftPath.top; i++) {
		cout << shortestLeftPath.sec[i - 1].i << ", " << shortestLeftPath.sec[i - 1].j << " --> " << shortestLeftPath.sec[i].i << ", " << shortestLeftPath.sec[i].j << endl;
		cvLine(img, cvPoint(left->sec[i - 1].i, left->sec[i - 1].j), cvPoint(left->sec[i].i, left->sec[i].j), cvScalar(0, 0, 255), 10);
	}
